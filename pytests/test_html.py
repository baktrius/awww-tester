...
def test_html_w3c(driver: WebDriver):
    ...
    for file in files:
        ...
        # if the validation failed, the "success" class won't be present
        # instead, the "failure" class will be present
        try:
            WebDriverWait(driver, 10).until(
                lambda d: d.find_element(By.CLASS_NAME, "success")
            )
        except:
...
def test_html_is_valid():
    ...
    if run(cmd, shell=True).returncode != 0:
        fd_input("Press enter to continue...")
        pytest.fail("html-validate failed.")
